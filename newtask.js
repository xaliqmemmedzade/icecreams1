...
const basketQuickCard = document.querySelector(".quick-basket-card")
const quickProdactselem = document.querySelector(".quickProducts")
...
console.log(icecreams.innerHTML);
...
search.addEventListener("input", () => {
  let filteredList = data.filter((item) => item.name.toLocaleLowerCase().includes(search.value.toLocaleLowerCase()));
...
basketBtn.addEventListener("click", () => {
  if (isQuickCard) {
    basketQuickCard.style.left = "-410px"
    isQuickCard = false
  } else {
    basketQuickCard.style.left = "0"
    isQuickCard = true
  }
})
...
function quickBasketRender(){
  quickProdactselem.innerHTML = ""
  cartList.forEach(item => {
    quickProdactselem.innerHTML += ` <li class="quick-product">
    <div>
      <h5>${item.name}</h5>
